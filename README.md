...
Output ini di-generate dari pipeline di repo induk `mbg/`:

```bash
# di repo induk (bukan di sini)
# 1. pastikan MCP user-xpoz aktif (checkAccessKeyStatus → hasAccessKey: true)
# 2. di Cursor / Claude Code:
"Jalankan dry-run MBG"   # cek volume dulu (~2 menit)
"Jalankan workflow MBG"  # full pipeline → hasil ke outputs/
```

Untuk menghasilkan dataset identik, lock:
- `config_hash = 144c95931fe3e84a`
- `llm_model = claude-opus-4.7`
- window `2025-08-01 → 2025-10-31`

Perubahan prompt / `theme_rules.yaml` / threshold akan mengubah
`config_hash` dan berpotensi mengubah label (sentimen & tema) — raw
data harusnya reproducible selama indeks xpoz tidak di-rebuild.

---

## Sitasi

> Luthfan, A. & Wisnu. (2026). *Text Mining X — Food Tray MBG*
> (Run `883a71c5`, config `144c95931fe3e84a`). Bagian dari skripsi
> "Redesign Food Tray MBG pada Sistem Distribusi SPPG".
> https://github.com/achmadluthfan/text-mining-x-food-tray-mbg

---

## Lisensi

Internal — skripsi Wisnu, 2026. Data post X yang di-include adalah
konten publik; gunakan sesuai [X Developer Agreement & Policy](https://developer.x.com/en/developer-terms/agreement-and-policy).
Hapus kolom PII jika akan dipublikasikan ulang.
